...
const { findWriteByHeadAndBody, findWriteByHeadAndBodyQuery } = require("../model/repository");
...
const findWrite = async (req, res) => {
  const { head, body } = req.body;

  const thisWrite = await findWriteByHeadAndBodyQuery(head, body);

  return res.status(200).json({ thisWrite });
};
